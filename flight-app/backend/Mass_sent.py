...
def apply_mt_sensitivity(
    df: pd.DataFrame,
    fuel_at_time_col: str = "Fuel_at_time_kg",
    deviation_percent: float = 0.0,
    phase_col: str = "flight_phase",
    phase_val: str = "Takeoff",
) -> tuple:
    """Apply MT sensitivity analysis using first row of specified phase as reference.

    Algorithm:
    1. Find the first row where flight_phase == phase_val (default "Takeoff")
    2. Get baseline mt value at that reference row
    3. Calculate adjusted_mt = baseline_mt × (1 + deviation_percent/100)
    4. Set mt at reference row to adjusted_mt
    5. Forward propagate (rows after reference): mt[i] = mt[i-1] - Fuel_at_time_kg[i]
    6. Backward propagate (rows before reference): mt[i] = mt[i+1] + Fuel_at_time_kg[i+1]

    Parameters:
    - df: Input DataFrame with flight_phase, Fuel_at_time_kg, and mt columns
    - fuel_at_time_col: Column name for fuel at each time point (default "Fuel_at_time_kg")
    - deviation_percent: Percentage deviation to apply (e.g., 5 for +5%, -10 for -10%)
    - phase_col: Column name for flight phase (default "flight_phase")
    - phase_val: Phase value to use as reference (default "Takeoff")

    Returns:
    - tuple: (df_modified, result_dict) where:
        - df_modified: DataFrame with updated mt column
        - result_dict: Dictionary containing sensitivity metadata
    """
    df_out = df.copy()
    try:
        # Ensure fuel_at_time_col exists
        if fuel_at_time_col not in df_out.columns:
            from Fuel import add_fuel_at_time_column
            df_out = add_fuel_at_time_column(df_out)
        ...
        # Find first row where phase_col == phase_val
        phases = df_out.get(phase_col)
        if phases is None:
            print(f"[ERROR] Column '{phase_col}' not found in DataFrame")
            result = {
                "status": "error",
                "message": f"Column '{phase_col}' not found",
                "deviation_percent": deviation_percent,
            }
            return df_out, result

        phase_mask = phases == phase_val
        true_indices = list(phase_mask[phase_mask].index)
        
        if len(true_indices) == 0:
            print(f"[ERROR] No rows found where {phase_col} == '{phase_val}'")
            result = {
                "status": "error",
                "message": f"No rows with {phase_col}='{phase_val}'",
                "deviation_percent": deviation_percent,
            }
            return df_out, result

        # Get integer position of first phase row
        phase_first_pos = int(np.where(phase_mask.values)[0][0])
        phase_first_row_idx = true_indices[0]

        # Get baseline mt at reference row
        if "mt" not in df_out.columns:
            # Add mt column if missing
            df_out = add_mt_column(df_out)
        
        baseline_mt = pd.to_numeric(df_out.loc[phase_first_row_idx, "mt"], errors="coerce")
        
        if pd.isna(baseline_mt):
            print(f"[ERROR] mt value at {phase_val} reference row is NA")
            result = {
                "status": "error",
                "message": "Baseline mt is NA at reference row",
                "deviation_percent": deviation_percent,
            }
            return df_out, result

        # Calculate adjusted mt
        adjustment_factor = 1.0 + (deviation_percent / 100.0)
        adjusted_mt = baseline_mt * adjustment_factor

        # Build mt array starting from adjusted value at reference position
        fuel_arr = fuel_filled.values.astype(float)
        mt_arr = np.full(n, np.nan, dtype=float)
        mt_arr[phase_first_pos] = float(adjusted_mt)

        # Forward propagation: mt[i] = mt[i-1] - fuel[i]
        for i in range(phase_first_pos + 1, n):
            if not np.isnan(mt_arr[i - 1]):
                mt_arr[i] = mt_arr[i - 1] - float(fuel_arr[i])

        # Backward propagation: mt[i] = mt[i+1] + fuel[i+1]
        for i in range(phase_first_pos - 1, -1, -1):
            if not np.isnan(mt_arr[i + 1]):
                mt_arr[i] = mt_arr[i + 1] + float(fuel_arr[i + 1])
        ...
        result = {
            ...
            "deviation_percent": deviation_percent,
            "phase_ref": phase_val,
            "phase_first_row_idx": int(phase_first_row_idx),
            "phase_first_pos": int(phase_first_pos),
            "baseline_mt_at_phase": float(baseline_mt),
            "adjusted_mt_at_phase": float(adjusted_mt),
            "adjustment_factor": float(adjustment_factor),
        }

        print(f"[OK] MT sensitivity applied: {phase_val} phase, baseline={baseline_mt:.2f} kg, "
              f"adjusted={adjusted_mt:.2f} kg (factor={adjustment_factor:.4f})")
        
        return df_out, result

    except Exception as e:
        print(f"[ERROR] Exception in apply_mt_sensitivity: {str(e)}")
        result = {
            "status": "error",
            "message": str(e),
            "deviation_percent": deviation_percent,
