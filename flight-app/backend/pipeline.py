...
from sen_temp_wind import compute_tas_for_dataframe
...
def _process_file(input_path: str, output_path: str, compute_tas: bool = False, aircraft_type: Optional[str] = None, target_mt0: Optional[float] = None) -> None:
    ...
    # 6.5 Optional: optimize mt0 to minimize Sumsq and optionally match takeoff weight
    # Try to use optimizer to find mt value that both minimizes aerodynamic
    # objective AND matches expected takeoff weight (if provided)
    try:
        if target_mt0 is not None:
            # If target_mt0 is provided, prioritize matching it
            print(f"Running optimize_mt0 with target_mt0={target_mt0}...")
            # weight_aero: importance of minimizing Sumsq (aerodynamic objective)
            # weight_target: importance of matching target_mt0 (MUCH higher priority)
            target_mt = target_mt0
            weight_aero = 0.01
            weight_target = 1000.0
        else:
            # If target_mt0 is NOT provided, use only aerodynamic objective (Sumsq)
    ...
    try:
        etow = df['mt'].iloc[0] if 'mt' in df.columns and len(df) > 0 else None
...
def process(input_path: str, output_path: str, compute_tas: bool = True, aircraft_type: Optional[str] = None, target_mt0: Optional[float] = None) -> None:
    ...
    if in_path.is_dir():
        ...
        for f in files:
            ...
            try:
                _process_file(f, str(dest), compute_tas=compute_tas, aircraft_type=aircraft_type, target_mt0=target_mt0)
            except Exception as e:
                print(f"Failed to process {fname}: {e}")
    else:
        # single file
        _process_file(str(in_path), str(out_path), compute_tas=compute_tas, aircraft_type=aircraft_type, target_mt0=target_mt0)


def main(argv: Optional[list] = None) -> int:
    argv = argv if argv is not None else sys.argv[1:]
    # support: python process_adsb_pipeline.py input.csv output.csv [aircraft_type] [target_mt0]
    if len(argv) >= 2:
        ...
        # If the fourth CLI arg (target_mt0) was provided, use it.
        target_mt0 = None
        if len(argv) >= 4:
            try:
                target_mt0 = float(argv[3])
            except ValueError:
                print(f"Warning: target_mt0 argument '{argv[3]}' is not a valid number, using default")
                target_mt0 = None
    else:
        ...
        try:
            ...
            atype = input("Aircraft type (e.g. 737, 320) [optional]: ").strip()
            aircraft_type = atype if atype != "" else None
            mt0_str = input("Target takeoff mass / mt0 [optional, default=57796]: ").strip()
            target_mt0 = float(mt0_str) if mt0_str != "" else None
        except Exception:
            print("Usage: python process_adsb_pipeline.py input.csv output.csv [aircraft_type] [target_mt0]")
            return 2

    process(input_path, output_path, aircraft_type=aircraft_type, target_mt0=target_mt0)
    return 0
